...
export type ProjectSectionId = "apps" | "tooling" | "experiments" | "templates" | "participation";
...
const GITHUB_API_URL = "https://api.github.com/users/Rostreet/repos?per_page=100&sort=updated";
...
const projectOverrides: Record<string, ProjectOverride> = {
	"rostreet.github.io": {
		...
		section: "apps",
		homepage: null,
		summary: "当前这座博客与作品站，基于 Next.js 16、App Router 与静态导出构建。",
	},
	NoteFlow: {
		featured: true,
		featuredRank: 2,
		section: "apps",
		summary: "面向记录与整理场景的笔记流产品原型，重点探索信息组织与写作体验。",
	},
	"IM-system": {
		featured: true,
		featuredRank: 3,
		section: "apps",
		summary: "用 Go 实现的即时通讯系统实验，关注聊天链路、消息传输与服务端组织。",
	},
	"Taichi-UI": {
		featured: true,
		featuredRank: 4,
		section: "tooling",
		summary: "面向 AI 场景的组件库与 playground，聚焦组件设计、可扩展性与交互体验。",
	},
	Rostreet: {
		section: "templates",
		homepage: null,
	...
	"Mini-react": {
		section: "experiments",
		summary: "用极少代码拆解 React 的基础能力，聚焦实现思路与最小功能闭环。",
	},
	ShishanForum: {
		section: "apps",
	...
	"langchain-selflearn": {
		section: "experiments",
		summary: "围绕 LangChain 的自学与实验仓库，记录 Agent 与 LLM 应用实践。",
	},
	...
	"neovate-code": {
		...
		summary: "参与 AI code agent 相关项目实践，关注代码生成、修复与自动化开发体验。",
	},
	"Element-Plus-X": {
		section: "participation",
	},
...
const fallbackRepos: GitHubProjectRepo[] = [
	...
	},
	{
		...
		description: "🚀 Vue3 + Element-Plus 开箱即用的企业级AI组件库前端解决方案 | Element-Plus-X ",
...
function inferSection(repo: GitHubProjectRepo): ProjectSectionId {
	...
	if (name.includes("mini") || name.includes("handwritten") || name.includes("langchain")) {
		return "experiments";
	}
...
function buildFallbackSummary(repo: GitHubProjectRepo, section: ProjectSectionId): string {
	if (repo.description) {
...
function normalizeProjects(repos: GitHubProjectRepo[]): Project[] {
	return repos
		...
		.map((repo) => {
			...
			const summary = override?.summary ?? buildFallbackSummary(repo, section);
			...
			return {
				...
				homepage: override && "homepage" in override ? (override.homepage ?? null) : repo.homepage,
			};
		});
}
