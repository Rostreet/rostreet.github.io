...
const sectionIcons: Record<ProjectSectionId, ComponentType<{ className?: string }>> = {
	apps: Rocket,
	tooling: Wrench,
	experiments: Code2,
...
function ProjectActions({ project }: { project: Project }) {
	const previewUrl =
		project.homepage && project.homepage !== project.html_url ? project.homepage : null;
		...
		<article className="border-b border-border/40 py-6">
			<div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
				...
				<div className="shrink-0 space-y-4 text-sm lg:text-right">
					<p className="text-muted-foreground">最近更新于 {formatDate(project.updated_at)}</p>
...
export default async function ProjectsPage() {
	...
	const projectGroups = projectSections.map((section) => ({
		...section,
		projects: projects.filter((project) => project.section === section.id && !project.featured),
	}));
	const independentProjects = projects.filter((project) => !project.fork).length;
			...
			<div className="mx-auto max-w-5xl">
				<header className="animate-fade-in">
					...
					<p className="mt-4 max-w-3xl text-[15px] leading-7 text-muted-foreground">
						这里收录了我在 GitHub 上的公开项目，按产品、工具、模板和底层实验做了整理。
					</p>
					...
					<div className="mt-6 flex flex-wrap gap-x-5 gap-y-1.5 text-sm text-muted-foreground">
						...
						<Link href="/blog" className="inline-flex items-center gap-2 hover:text-foreground">
							Blog
							<ArrowUpRight className="h-4 w-4" />
						</Link>
					</div>
